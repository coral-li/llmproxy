...
class TestProxyLive:
    ...
    def proxy_url(self):
        """Get proxy URL and verify it's running"""
        proxy_url = get_proxy_url()
        
        # Check if proxy is running
        try:
            print(f"Checking proxy health at {proxy_url}/health")
            response = httpx.get(f"{proxy_url}/health", timeout=5)
            if response.status_code != 200:
                pytest.fail(f"Proxy health check failed: {response.status_code}")
        except Exception as e:
            pytest.fail(f"Cannot connect to proxy at {proxy_url}: {e}")
        
        return proxy_url

    @pytest.fixture(scope="session")
    def openai_client(self, proxy_url):
        """Create OpenAI client pointing to proxy"""
        return OpenAI(base_url=proxy_url)

    @pytest.fixture(scope="session")
    def async_openai_client(self, proxy_url):
        """Create AsyncOpenAI client pointing to proxy"""
        return AsyncOpenAI(base_url=proxy_url)

    @pytest.fixture(scope="session")
    def model(self, proxy_url):
    ...
    @pytest.fixture(scope="session", autouse=True)
    def clear_cache(self, proxy_url):
        """Clear all caches before starting tests"""
        async def _clear_cache():
            print("Invalidating all caches to start with clean state...")
            try:
                async with httpx.AsyncClient() as client:
                    response = await client.delete(f"{proxy_url}/cache", timeout=10.0)
                    
                    if response.status_code == 200:
                        data = response.json()
                        deleted_count = data.get('deleted_entries', 0)
                        print(f"✅ Cache invalidation successful: {deleted_count} entries deleted")
                    else:
                        print(f"⚠️  Cache invalidation returned status {response.status_code}: {response.text}")
    ...
    def test_basic_completion(self, openai_client, model):
        ...
        assert "Hello from LLMProxy!" in content, f"Expected greeting not found in: {content}"
    ...
    def test_streaming(self, openai_client, model):
        ...
        assert numbers_found >= 3, f"Expected at least 3 numbers (1-5) in response, found {numbers_found}: {full_response}"
    ...
    def test_caching(self, openai_client, model):
        ...
        assert duration2 < duration1 * 0.8, f"Second request not significantly faster: {duration1:.2f}s vs {duration2:.2f}s"
    ...
    def test_error_handling(self, openai_client):
        ...
        # Assertion
        assert "invalid-model-name" in error_msg.lower() or "model" in error_msg.lower(), f"Error message should mention invalid model: {error_msg}"
    ...
    def test_streaming_caching(self, openai_client, model):
        ...
        assert content_matches, f"Cached content doesn't match:\nFirst: {content1}\nSecond: {content2}"
        assert cache_worked, f"Cache didn't provide significant speedup: {duration1:.2f}s vs {duration2:.2f}s"
    ...
    def test_responses_api_streaming(self, openai_client, model):
        ...
        self.log(f"✅ Responses API streaming success: {len(chunks)} chunks received (took {duration:.2f}s)")
        self.log(f"   Response: {full_response}")
    ...
    def test_responses_api_streaming_cache(self, openai_client, model):
        ...
        # Assertions
        assert len(chunks1) > 0, "First responses API streaming request produced no chunks"
        assert len(chunks2) > 0, "Second responses API streaming request produced no chunks"
        assert content_similar, f"Cached content not similar enough:\nFirst: {content1}\nSecond: {content2}"
        assert cache_worked, f"Cache didn't provide significant speedup: {duration1:.2f}s vs {duration2:.2f}s"
    ...
    def test_load_balancing(self, openai_client, model):
        ...
        for i in range(num_requests):
            ...
            assert response.choices[0].message.content, f"Request {i+1} has no content"
            assert f"Request {i+1}" in response.choices[0].message.content, f"Request {i+1} has unexpected content"
        ...
        # Assertions
        assert len(request_times) == num_requests, f"Not all requests succeeded: {len(request_times)}/{num_requests}"
    ...
    async def test_concurrent_requests(self, proxy_url, model):
        ...
        # Assertions
        assert len(successful) >= num_concurrent // 2, f"Too many concurrent requests failed: {len(successful)}/{num_concurrent}"
        for result in successful:
            assert f"Concurrent {result['index']}" in result['content'], f"Unexpected content in concurrent request {result['index']}"
    ...
    async def test_health_and_stats(self, proxy_url):
        ...
        async with httpx.AsyncClient() as client:
            ...
            assert stats_resp.status_code == 200, f"Stats endpoint returned {stats_resp.status_code}"
    ...
    async def test_responses_api_async(self, async_openai_client, model):
        ...
        assert "unicorn" in streaming_output.lower(), f"Expected 'unicorn' in streaming response: {streaming_output}"
