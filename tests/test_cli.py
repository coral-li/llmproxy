...
class TestCLI:
    ...
    def test_main_with_default_args(self):
        ...
        with patch("sys.argv", ["llmproxy"]), patch(
            "llmproxy.cli.load_config"
        ) as mock_load_config, patch("uvicorn.Server.run") as mock_run, patch(
    ...
    def test_main_with_host_port_override(self):
        ...
        ), patch("llmproxy.cli.load_config") as mock_load_config, patch(
            "uvicorn.Server.run"
        ) as mock_run, patch(
            "llmproxy.cli.setup_logging"
        ), patch(
    ...
    def test_main_with_log_level(self):
        ...
        for log_level in log_levels:
            with patch("sys.argv", ["llmproxy", "--log-level", log_level]), patch(
                "llmproxy.cli.load_config"
            ) as mock_load_config, patch("uvicorn.Server.run"), patch(
                "llmproxy.cli.setup_logging"
            ) as mock_setup_logging, patch(
                "os.path.exists", return_value=False
            ):
    ...
    def test_main_handles_general_exception(self):
        ...
        with patch("sys.argv", ["llmproxy"]), patch(
            "llmproxy.cli.load_config", side_effect=Exception("General error")
        ), patch("llmproxy.cli.setup_logging"), patch(
            "os.path.exists", return_value=False
        ), patch(
            "sys.exit"
